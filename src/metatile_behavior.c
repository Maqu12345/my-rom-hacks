...
static const u8 sTileBitAttributes[NUM_METATILE_BEHAVIORS] =
{
    ...
    [MB_HIDDEN_GRASS]                          = TILE_FLAG_UNUSED | TILE_FLAG_HAS_ENCOUNTERS,
    ...
    [MB_SEAWEED_LAND]                       = TILE_FLAG_UNUSED | TILE_FLAG_HAS_ENCOUNTERS,
...
bool8 MetatileBehavior_IsHiddenGrass(u8 metatileBehavior)
{
    if (metatileBehavior == MB_HIDDEN_GRASS)
        return TRUE;
    else
        return FALSE;
}
...
{
    if (metatileBehavior == MB_SEAWEED || metatileBehavior == MB_SEAWEED_LAND || metatileBehavior == MB_SEAWEED_NO_SURFACING)
        return TRUE;
    else
        return FALSE;
}
