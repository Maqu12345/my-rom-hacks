...
bool32 TryRunFromBattle(u32 battler)
{
    ...
    else if (B_GHOSTS_ESCAPE >= GEN_6 && IS_BATTLER_OF_TYPE(battler, TYPE_NETHER))
    {
        effect++;
    }
...
u32 TrySetCantSelectMoveBattleScript(u32 battler)
{
    ...
    if (DYNAMAX_BYPASS_CHECK && GetActiveGimmick(battler) != GIMMICK_Z_MOVE && gDisableStructs[battler].throatChopTimer > gBattleTurnCounter && GetBattleMoveType(move) == TYPE_SOUND)
    {
        gCurrentMove = move;
        if (gBattleTypeFlags & BATTLE_TYPE_PALACE)
        {
...
u8 CheckMoveLimitations(u32 battler, u8 unusableMoves, u16 check)
{
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        ...
        // Belch
            ...
            unusableMoves |= 1u << i;
        // Throat Chop
        else if (check & MOVE_LIMITATION_THROAT_CHOP && gDisableStructs[battler].throatChopTimer > gBattleTurnCounter && GetBattleMoveType(move) == TYPE_SOUND)
            unusableMoves |= 1u << i;
        // Stuff Cheeks
...
{
    if (GetActiveGimmick(gBattlerAttacker) != GIMMICK_Z_MOVE && gDisableStructs[gBattlerAttacker].throatChopTimer > gBattleTurnCounter && GetBattleMoveType(gCurrentMove) == TYPE_SOUND)
    {
        gProtectStructs[gBattlerAttacker].unableToUseMove = TRUE;
        CancelMultiTurnMoves(gBattlerAttacker, SKY_DROP_ATTACKCANCELLER_CHECK);
...
bool32 ChangeTypeBasedOnTerrain(u32 battler)
{
    ...
    if (gFieldStatuses & STATUS_FIELD_ELECTRIC_TERRAIN)
        ...
        battlerType = TYPE_WATER;
    else if (gFieldStatuses & STATUS_FIELD_PSYCHIC_TERRAIN)
        battlerType = TYPE_WARPED;
    else // failsafe
        return FALSE;
...
bool32 CanAbilityBlockMove(u32 battlerAtk, u32 battlerDef, u32 abilityAtk, u32 abilityDef, u32 move, enum AbilityEffectOptions option)
{
    ...
    case ABILITY_SOUNDPROOF:
        if (GetBattleMoveType(move) == TYPE_SOUND && !(GetBattlerMoveTargetType(battlerAtk, move) & MOVE_TARGET_USER))
        {
...
bool32 CanAbilityAbsorbMove(u32 battlerAtk, u32 battlerDef, u32 abilityDef, u32 move, u32 moveType, enum AbilityEffectOptions option)
{
    ...
    case ABILITY_WIND_RIDER:
        if (moveType == TYPE_WIND && !(GetBattlerMoveTargetType(battlerAtk, move) & MOVE_TARGET_USER))
        {
            effect = MOVE_ABSORBED_BY_STAT_INCREASE_ABILITY;
            statId = STAT_ATK;
        }
    ...
    case ABILITYEFFECT_MOVE_END: // Think contact abilities.
        ...
        case ABILITY_RATTLED:
            if (!(gBattleStruct->moveResultFlags[battler] & MOVE_RESULT_NO_EFFECT)
             && IsBattlerTurnDamaged(gBattlerTarget)
             && IsBattlerAlive(battler)
             && (moveType == TYPE_DARK || moveType == TYPE_SOUND || moveType == TYPE_NETHER)
...
{
    if (B_GHOSTS_ESCAPE >= GEN_6 && IS_BATTLER_OF_TYPE(battler, TYPE_NETHER))
        return 0;
...
bool32 CanSetNonVolatileStatus(u32 battlerAtk, u32 battlerDef, u32 abilityAtk, u32 abilityDef, enum MoveEffects effect, enum NonVolatileStatus option)
{
    ...
    case MOVE_EFFECT_SLEEP:
        ...
        else if (IS_BATTLER_OF_TYPE(battlerDef, TYPE_DREAM))
        {
            battleScript = BattleScript_NotAffected;
        }
    ...
    case MOVE_EFFECT_FROSTBITE:
        ...
        else if (IS_BATTLER_OF_TYPE(battlerDef, TYPE_WATER) || IsBattlerWeatherAffected(battlerDef, B_WEATHER_SUN))
        {
            battleScript = BattleScript_NotAffected;
        }
...
u32 ItemBattleEffects(enum ItemCaseId caseID, u32 battler, bool32 moveTurn)
{
    ...
    case ITEMEFFECT_LIFEORB_SHELLBELL:
        ...
        case HOLD_EFFECT_THROAT_SPRAY:  // Does NOT need to be a damaging move
            if (GetBattleMoveType(gCurrentMove) == TYPE_SOUND
             && IsBattlerAlive(gBattlerAttacker)
...
const struct TypePower gNaturalGiftTable[] =
{
    ...
    [ITEM_TO_BERRY(ITEM_PECHA_BERRY)] = {TYPE_NATURE, 80},
    [ITEM_TO_BERRY(ITEM_RAWST_BERRY)] = {TYPE_EARTH, 80},
    [ITEM_TO_BERRY(ITEM_ASPEAR_BERRY)] = {TYPE_STEEL, 80},
    [ITEM_TO_BERRY(ITEM_LEPPA_BERRY)] = {TYPE_WIND, 80},
    [ITEM_TO_BERRY(ITEM_ORAN_BERRY)] = {TYPE_ELECTRIC, 80},
    [ITEM_TO_BERRY(ITEM_PERSIM_BERRY)] = {TYPE_LIGHT, 80},
    [ITEM_TO_BERRY(ITEM_LUM_BERRY)] = {TYPE_DARK, 80},
    [ITEM_TO_BERRY(ITEM_SITRUS_BERRY)] = {TYPE_NETHER, 80},
    [ITEM_TO_BERRY(ITEM_FIGY_BERRY)] = {TYPE_POISON, 80},
    [ITEM_TO_BERRY(ITEM_WIKI_BERRY)] = {TYPE_FIGHTING, 80},
    [ITEM_TO_BERRY(ITEM_MAGO_BERRY)] = {TYPE_ILLUSION, 80},
    [ITEM_TO_BERRY(ITEM_AGUAV_BERRY)] = {TYPE_SOUND, 80},
    [ITEM_TO_BERRY(ITEM_IAPAPA_BERRY)] = {TYPE_WARPED, 80},
    [ITEM_TO_BERRY(ITEM_CHILAN_BERRY)] = {TYPE_VOID, 80},
    ...
    [ITEM_TO_BERRY(ITEM_KASIB_BERRY)] = {TYPE_NETHER, 80},
    [ITEM_TO_BERRY(ITEM_KEBIA_BERRY)] = {TYPE_POISON, 80},
    [ITEM_TO_BERRY(ITEM_CHOPLE_BERRY)] = {TYPE_FIGHTING, 80},
    [ITEM_TO_BERRY(ITEM_PAYAPA_BERRY)] = {TYPE_ILLUSION, 80},
    [ITEM_TO_BERRY(ITEM_HABAN_BERRY)] = {TYPE_SOUND, 80},
    [ITEM_TO_BERRY(ITEM_YACHE_BERRY)] = {TYPE_WARPED, 80},
    [ITEM_TO_BERRY(ITEM_RAZZ_BERRY)] = {TYPE_FIRE, 90},
    [ITEM_TO_BERRY(ITEM_BLUK_BERRY)] = {TYPE_WATER, 90},
    [ITEM_TO_BERRY(ITEM_NANAB_BERRY)] = {TYPE_NATURE, 90},
    [ITEM_TO_BERRY(ITEM_WEPEAR_BERRY)] = {TYPE_EARTH, 90},
    [ITEM_TO_BERRY(ITEM_PINAP_BERRY)] = {TYPE_STEEL, 90},
    [ITEM_TO_BERRY(ITEM_POMEG_BERRY)] = {TYPE_WIND, 90},
    [ITEM_TO_BERRY(ITEM_KELPSY_BERRY)] = {TYPE_ELECTRIC, 90},
    [ITEM_TO_BERRY(ITEM_QUALOT_BERRY)] = {TYPE_LIGHT, 90},
    [ITEM_TO_BERRY(ITEM_HONDEW_BERRY)] = {TYPE_DARK, 90},
    [ITEM_TO_BERRY(ITEM_GREPA_BERRY)] = {TYPE_NETHER, 90},
    [ITEM_TO_BERRY(ITEM_TAMATO_BERRY)] = {TYPE_POISON, 90},
    [ITEM_TO_BERRY(ITEM_CORNN_BERRY)] = {TYPE_FIGHTING, 90},
    [ITEM_TO_BERRY(ITEM_MAGOST_BERRY)] = {TYPE_ILLUSION, 90},
    [ITEM_TO_BERRY(ITEM_RABUTA_BERRY)] = {TYPE_SOUND, 90},
    [ITEM_TO_BERRY(ITEM_NOMEL_BERRY)] = {TYPE_WARPED, 90},
    [ITEM_TO_BERRY(ITEM_SPELON_BERRY)] = {TYPE_FIRE, 100},
    [ITEM_TO_BERRY(ITEM_PAMTRE_BERRY)] = {TYPE_WATER, 100},
    [ITEM_TO_BERRY(ITEM_WATMEL_BERRY)] = {TYPE_NATURE, 100},
    [ITEM_TO_BERRY(ITEM_DURIN_BERRY)] = {TYPE_EARTH, 100},
    [ITEM_TO_BERRY(ITEM_BELUE_BERRY)] = {TYPE_STEEL, 100},
    [ITEM_TO_BERRY(ITEM_LIECHI_BERRY)] = {TYPE_WIND, 100},
    [ITEM_TO_BERRY(ITEM_GANLON_BERRY)] = {TYPE_ELECTRIC, 100},
    [ITEM_TO_BERRY(ITEM_SALAC_BERRY)] = {TYPE_LIGHT, 100},
    [ITEM_TO_BERRY(ITEM_PETAYA_BERRY)] = {TYPE_DARK, 100},
    [ITEM_TO_BERRY(ITEM_APICOT_BERRY)] = {TYPE_NETHER, 100},
    [ITEM_TO_BERRY(ITEM_LANSAT_BERRY)] = {TYPE_POISON, 100},
    [ITEM_TO_BERRY(ITEM_STARF_BERRY)] = {TYPE_FIGHTING, 100},
    [ITEM_TO_BERRY(ITEM_ENIGMA_BERRY)] = {TYPE_DREAM, 100},
    [ITEM_TO_BERRY(ITEM_MICLE_BERRY)] = {TYPE_ILLUSION, 100},
    [ITEM_TO_BERRY(ITEM_CUSTAP_BERRY)] = {TYPE_SOUND, 100},
    [ITEM_TO_BERRY(ITEM_JABOCA_BERRY)] = {TYPE_WARPED, 100},
    [ITEM_TO_BERRY(ITEM_ROWAP_BERRY)] = {TYPE_FIRE, 100},
    [ITEM_TO_BERRY(ITEM_KEE_BERRY)] = {TYPE_WATER, 100},
    [ITEM_TO_BERRY(ITEM_MARANGA_BERRY)] = {TYPE_NATURE, 100},
    [ITEM_TO_BERRY(ITEM_TANGA_BERRY)] = {TYPE_DREAM, 80},
    [ITEM_TO_BERRY(ITEM_CHARTI_BERRY)] = {TYPE_DREAM, 90},
};
...
static inline u32 CalcMoveBasePowerAfterModifiers(struct DamageCalculationData *damageCalcData, u32 atkAbility, u32 defAbility, enum ItemHoldEffect holdEffectAtk, u32 weather)
{
    ...
    if (IsBattlerTerrainAffected(battlerAtk, STATUS_FIELD_ELECTRIC_TERRAIN) && moveType == TYPE_ELECTRIC)
        modifier = uq4_12_multiply(modifier, (B_TERRAIN_TYPE_BOOST >= GEN_8 ? UQ_4_12(1.3) : UQ_4_12(1.5)));
    if (IsBattlerTerrainAffected(battlerAtk, STATUS_FIELD_PSYCHIC_TERRAIN) && moveType == TYPE_WARPED)
        modifier = uq4_12_multiply(modifier, (B_TERRAIN_TYPE_BOOST >= GEN_8 ? UQ_4_12(1.3) : UQ_4_12(1.5)));
    ...
    case ABILITY_SAND_FORCE:
        if ((moveType == TYPE_STEEL || moveType == TYPE_EARTH)
    ...
    case ABILITY_NORMALIZE:
        if (moveType == TYPE_VOID && gBattleStruct->ateBoost[battlerAtk] && GetGenConfig(GEN_CONFIG_ATE_MULTIPLIER) >= GEN_7)
            modifier = uq4_12_multiply(modifier, UQ_4_12(1.2));
        break;
    ...
    // field abilities
    if ((IsAbilityOnField(ABILITY_DARK_AURA) && moveType == TYPE_NETHER)
    ...
    case HOLD_EFFECT_LUSTROUS_ORB:
        if (GET_BASE_SPECIES_ID(gBattleMons[battlerAtk].species) == SPECIES_PALKIA && (moveType == TYPE_WATER || moveType == TYPE_WARPED))
            modifier = uq4_12_multiply(modifier, holdEffectModifier);
        break;
    case HOLD_EFFECT_ADAMANT_ORB:
        if (GET_BASE_SPECIES_ID(gBattleMons[battlerAtk].species) == SPECIES_DIALGA && (moveType == TYPE_STEEL || moveType == TYPE_WARPED))
            modifier = uq4_12_multiply(modifier, holdEffectModifier);
        break;
    case HOLD_EFFECT_GRISEOUS_ORB:
        if (GET_BASE_SPECIES_ID(gBattleMons[battlerAtk].species) == SPECIES_GIRATINA && (moveType == TYPE_NETHER || moveType == TYPE_WARPED))
            modifier = uq4_12_multiply(modifier, holdEffectModifier);
        break;
    case HOLD_EFFECT_SOUL_DEW:
        if ((gBattleMons[battlerAtk].species == SPECIES_LATIAS || gBattleMons[battlerAtk].species == SPECIES_LATIOS)
            && ((B_SOUL_DEW_BOOST >= GEN_7 && (moveType == TYPE_ILLUSION || moveType == TYPE_LIGHT || moveType == TYPE_WIND))
...
static inline u32 CalcAttackStat(struct DamageCalculationData *damageCalcData, u32 atkAbility, u32 defAbility, enum ItemHoldEffect holdEffectAtk, u32 weather)
{
    ...
    case ABILITY_FLASH_FIRE:
        ...
        break;
    case ABILITY_SWARM:
        if (moveType == TYPE_POISON && gBattleMons[battlerAtk].hp <= (gBattleMons[battlerAtk].maxHP / 3))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
        break;
...
static inline uq4_12_t GetDefenderItemsModifier(struct DamageCalculationData *damageCalcData, uq4_12_t typeEffectivenessModifier, u32 abilityDef, enum ItemHoldEffect holdEffectDef)
{
    ...
    case HOLD_EFFECT_RESIST_BERRY:
        if (UnnerveOn(battlerDef, itemDef))
            return UQ_4_12(1.0);
        if (moveType == holdEffectDefParam && (moveType == TYPE_VOID || typeEffectivenessModifier >= UQ_4_12(2.0)))
        {
            if (damageCalcData->updateFlags)
...
static inline void MulByTypeEffectiveness(uq4_12_t *modifier, u32 move, u32 moveType, u32 battlerDef, u32 defAbility, u32 defType, u32 battlerAtk, bool32 recordAbilities)
{
    ...
    else if ((moveType == TYPE_VOID) && defType == TYPE_ILLUSION && gBattleMons[battlerDef].status2 & STATUS2_FORESIGHT && mod == UQ_4_12(0.0))
    {
        mod = UQ_4_12(1.0);
    }
    else if ((abilityAtk == ABILITY_SCRAPPY || abilityAtk == ABILITY_MINDS_EYE)
        && mod == UQ_4_12(0.0))
    {
        mod = UQ_4_12(1.0);
        if (recordAbilities)
    ...
    if (moveType == TYPE_ILLUSION && defType == TYPE_VOID && gStatuses3[battlerDef] & STATUS3_MIRACLE_EYED && mod == UQ_4_12(0.0))
        mod = UQ_4_12(1.0);
...
static inline uq4_12_t CalcTypeEffectivenessMultiplierInternal(u32 move, u32 moveType, u32 battlerAtk, u32 battlerDef, bool32 recordAbilities, uq4_12_t modifier, u32 defAbility)
{
    ...
    if (((defAbility == ABILITY_WONDER_GUARD && modifier <= UQ_4_12(1.0) && moveType != TYPE_DREAM)
...
uq4_12_t CalcPartyMonTypeEffectivenessMultiplier(u16 move, u16 speciesDef, u16 abilityDef)
{
    ...
    if (move != MOVE_STRUGGLE && moveType != TYPE_MYSTERY)
    {
        ...
        if (moveType == TYPE_EARTH && abilityDef == ABILITY_LEVITATE && !(gFieldStatuses & STATUS_FIELD_GRAVITY))
            modifier = UQ_4_12(0.0);
        if (abilityDef == ABILITY_WONDER_GUARD && modifier <= UQ_4_12(1.0) && GetMovePower(move) != 0 && moveType != TYPE_DREAM)
            modifier = UQ_4_12(0.0);
    }
...
uq4_12_t GetOverworldTypeEffectiveness(struct Pokemon *mon, u8 moveType)
{
    ...
    if (moveType != TYPE_MYSTERY || moveType != TYPE_DREAM)
    {
...
bool32 BlocksPrankster(u16 move, u32 battlerPrankster, u32 battlerDef, bool32 checkTarget)
{
    ...
    if (checkTarget && (GetBattlerMoveTargetType(battlerPrankster, move) & (MOVE_TARGET_OPPONENTS_FIELD | MOVE_TARGET_DEPENDS)))
        return FALSE;
    if (!IS_BATTLER_OF_TYPE(battlerDef, TYPE_ILLUSION))
        return FALSE;
...
{
    enum BattleMoveEffects effect = GetMoveEffect(move);
    if (effect == EFFECT_CURSE && !IS_BATTLER_OF_TYPE(battler, TYPE_NETHER))
...
void GetBattlerTypes(u32 battler, bool32 ignoreTera, u32 types[static 3])
{
    ...
    if (!isTera && gDisableStructs[battler].roostActive)
    {
        if (types[0] == TYPE_WIND && types[1] == TYPE_WIND)
            types[0] = types[1] = B_ROOST_PURE_FLYING >= GEN_5 ? TYPE_VOID : TYPE_MYSTERY;
